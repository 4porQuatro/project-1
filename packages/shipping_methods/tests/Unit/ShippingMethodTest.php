...
class ShippingMethodTest extends TestCase
{
    ...
    public function its_possible_retrieve_the_the_prices_for_a_given_country_volume_and_weight()
    {
        ...
        $shipping_price = ShippingPrice::factory()
            ->create(['min_volume'=>5, 'max_volume'=>10, 'price'=>10, 'free_order_price'=>100, 'priceable_type'=>Zone::class, 'priceable_id'=>$zone1->id, 'shipping_method_id'=>$shipping_method->id]);

        $shipping_method->setShippingPriceByAttributesAndCountry($country_france->id, 7, 500);

        $this->assertEquals(10, $shipping_method->price);
        $this->assertEquals(100, $shipping_method->free_order_price);
